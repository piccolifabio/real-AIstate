...
export default function ImmobilePage() {
  ...
  // Lista tipi documento estratta in src/constants/documentTypes.js: template
  // proposta sempre disponibile + 6 doc tipo mostrati come card "Su richiesta".
  // Quando avremo la tabella documenti_immobile (post-MVP), questi item
  // diventeranno template rispetto a cui mostrare uploaded vs missing.
  // Comparabili restano hardcoded solo per Capecelatro (demo perfetta) finché
  // non ci sarà tabella DB dedicata.
            ...
            <div className="docs-section">
              ...
              {!user && (
                <div style={{ background: "rgba(217,48,37,0.08)", border: "1px solid rgba(217,48,37,0.2)", borderRadius: 3, padding: "1.2rem 1.5rem", marginBottom: "1rem", display: "flex", alignItems: "center", gap: "1rem" }}>
                  ...
                  <div>
                    <div style={{ fontSize: "0.82rem", fontWeight: 600, color: "#f7f5f0", marginBottom: "0.3rem" }}>Accedi per richiedere i documenti</div>
                    <div style={{ fontSize: "0.78rem", color: "rgba(247,245,240,0.5)" }}>
                      Registrati gratuitamente per richiedere al venditore visura, planimetria, APE e altri documenti.{" "}
              ...
              {/* Template proposta — sempre disponibile, anche per anonimi (è un PDF pubblico). */}
              <div style={{ background: "var(--surface)", border: "1px solid var(--border)", borderRadius: 2, padding: "1rem", marginBottom: "1rem", display: "flex", alignItems: "center", gap: "0.8rem" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: "0.82rem", fontWeight: 500, color: "var(--white)", marginBottom: "0.2rem" }}>{PROPOSAL_TEMPLATE.label}</div>
                  <div style={{ fontSize: "0.68rem", letterSpacing: "0.06em", textTransform: "uppercase", color: "var(--green-light)" }}>Disponibile</div>
                  <a href={PROPOSAL_TEMPLATE.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: "0.7rem", color: "var(--red)", fontWeight: 600, textDecoration: "none", display: "block", marginTop: "0.3rem" }}>↓ Visualizza documento</a>
                </div>
              </div>

              {/* Documenti tipo — mostrati come card "Su richiesta" finché non avremo
                  upload reali per-immobile (tabella documenti_immobile post-MVP). */}
              <div className="docs-grid">
                {DOCUMENT_TYPES.map((doc) => (
                  <div className="doc-item" key={doc.key}>
                    <div className="doc-icon missing">🔒</div>
                    <div className="doc-info">
                      <div className="doc-name">{doc.label}</div>
                      <div className="doc-status ko">Su richiesta</div>
                      <div style={{ fontSize: "0.7rem", color: "var(--muted)", marginTop: "0.3rem" }}>
                        {user ? "Richiedi via chat al venditore" : "🔒 Accedi per richiedere"}
                      </div>
                    </div>
                  </div>
                ))}
